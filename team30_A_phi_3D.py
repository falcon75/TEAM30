...
def solve_team30(single_phase: bool, num_phases: int, omega_u: np.float64, degree: np.int32, petsc_options: dict = {},
    ...
    FE_scalar = ufl.FiniteElement("Lagrange", cell, degree)
    FE_vector = ufl.VectorElement("Lagrange", cell, degree)
    ...
    # FIXME: Vector current in future
    ...
    for i in range(num_steps):
        ...
        # Compute losses, torque and induced voltage, not updated to 3D
        ...
        if save_output:
            # post_B.interpolate()
            A_out.x.array[:] = AV.sub(0).collapse().x.array[:]
            A_vtx.write(t)
            B_vtx.write(t)
...
if __name__ == "__main__":
    ...
    if args.single:
        outdir = f"TEAM30_3D_{args.omegaU}_single"
    ...
    if args.three:
        outdir = f"TEAM30_3D_{args.omegaU}_three"
        os.system(f"mkdir -p {outdir}")
